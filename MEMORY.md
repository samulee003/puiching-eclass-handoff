...
更新日期：2026-09-18（Asia/Macau）。改行為或改資料的 agent 必須同步更新本檔日期與受影響段落。

## 一句話現況

全自動雙孩 eClass 爬蟲工作流（GitHub Actions 定時同步與手動觸發）運作正常！2026-09-18 成功修復並復原 workflow 檔案，雙孩（李悅 P3、李昕 P1）各自憑證經 GitHub Secrets 登入，實時自真實培正 eClass 抓取並原子雙寫至 `status.json` 與 `DASHBOARD.md`（共 23 項紀錄）。李悅今日必做英文造句（Writing Journal），李昕今日無必須繳交項目，全套 212 項端對端測試 100% 通過！
...
2. **M2 eClass 自動抓取引擎與 GitHub Actions 零額度排程（已全面實裝並通過實測）**：
   ...
   - 業務過濾規則實測：
     ...
     - 日期分類：以「限期」計算剩餘天數，精準放入「今日必做」、「近幾日」、「測驗」或其他。
...
4. **客觀驗收測試（212/212 100% Pass）**：
   - 本機與 CI 均能運行 `pytest`，涵蓋 Tier 1 功能測項（143 項）、Tier 2 邊界異常測項（50 項）、Tier 3 跨功能整合測項（5 項）、Tier 4 真實生活場景測項（14 項）。

## Git 分支與提交進度

- 當前分支：`main`
- 最新狀態：
   - **2026-09-18 事故修復與雙孩實時抓取**：
     1. 完整修復與復原 `.github/workflows/eclass-sync.yml`（先前提交因缺乏 workflow 授權導致丟失，現以具備 workflow scope 憑證推回 `main` 並成功啟用）。
     2. 透過 GitHub Actions 執行雙孩實時抓取（Run ID: `35340620042`），李悅（P3）與李昕（P1）實時家課經 eClass 爬蟲原子雙寫至 `status.json` 與 `DASHBOARD.md`。
     3. 李悅今日必做：英文 `(Ms. Au Ieong) Do sentence making (Writing Journal)`；李昕今日無必須繳交項目。
     4. 全套回歸測試 212/212 項測項全數通過（100% Pass）。
